...
public class VertexInvoicePluginApiITest extends VertexRemoteTestBase {
    ...
    public void testItemAdjustments() {
        ...
        List<InvoiceItem> additionalInvoiceItems = vertexInvoicePluginApi.getAdditionalInvoiceItems(invoice, false, pluginProperties, new InvoiceContextImp.Builder<>().withInvoice(invoice).withIsDryRun(false).withTenantId(callContext.getTenantId()).build()).getAdditionalItems();
        ...
        additionalInvoiceItems = vertexInvoicePluginApi.getAdditionalInvoiceItems(invoice, false, pluginProperties, new InvoiceContextImp.Builder<>().withInvoice(invoice).withIsDryRun(false).withTenantId(callContext.getTenantId()).build()).getAdditionalItems();
    ...
    public void testRepair() throws Exception {
        ...
        pluginProperties.add(new PluginProperty(String.format("%s_%s", VertexTaxCalculator.TAX_CODE, taxableItem1.getId()), "D9999999", false));
        List<InvoiceItem> additionalInvoiceItems = vertexInvoicePluginApi.getAdditionalInvoiceItems(invoice1, false, pluginProperties, new InvoiceContextImp.Builder<>().withInvoice(invoice1).withIsDryRun(false).withTenantId(callContext.getTenantId()).build()).getAdditionalItems();
        // TAX expected (total $8.63)
        checkTaxes(additionalInvoiceItems, new BigDecimal("8.63"));
        ...
        additionalInvoiceItems = vertexInvoicePluginApi.getAdditionalInvoiceItems(invoice1, false, pluginProperties, new InvoiceContextImp.Builder<>().withInvoice(invoice1).withIsDryRun(false).withTenantId(callContext.getTenantId()).build()).getAdditionalItems();
        checkTaxes(additionalInvoiceItems, BigDecimal.ZERO);
        ...
        Mockito.when(osgiKillbillAPI.getInvoiceUserApi().getInvoiceByInvoiceItem(Mockito.eq(taxableItem1.getId()), Mockito.any()))
               .thenReturn(invoice1);
        additionalInvoiceItems = vertexInvoicePluginApi.getAdditionalInvoiceItems(invoice2, false, pluginProperties, new InvoiceContextImp.Builder<>().withInvoice(invoice1).withIsDryRun(false).withTenantId(callContext.getTenantId()).build()).getAdditionalItems();
        // TAX expected (total -$4.32)
        checkTaxes(additionalInvoiceItems, new BigDecimal("-4.31"));
